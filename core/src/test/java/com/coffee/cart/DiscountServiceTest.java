...
public class DiscountServiceTest {
    ...
    void whenNoDiscounts_ThenReturnNoDiscountResponse() {
        try (MockedStatic<CartPriceCalculationService> mockedPriceCalculation = mockStatic(CartPriceCalculationService.class)) {
            BigDecimal totalCartPrice = BigDecimal.valueOf(10);

            when(cartItemService.getCartItemList(userUid))
                    .thenReturn(cartItemList);
            mockedPriceCalculation.when(() -> getTotalCartPrice(cartItemList))
